...
\begin{verbatim}
#> %% R package xdvir_0.1.0; engine name: XeTeX; engine version: 3.14159265-2.6-0.99999
...
produces a \texttt{"DVI"} object that contains instructions specifying the fonts to
...
#>              comment=R package xdvir_0.1.0; engine name: XeTeX; engine version: 3.14
...
\LaTeX{} packages. As a simple example, changing the colour of text
...
\texttt{\textbackslash{}LaTeX\{\}package()} function. We will see other predefined packages
...
\includegraphics[width=1\linewidth]{murrell-xdvir_files/figure-latex/unnamed-chunk-32-1}
...
\includegraphics[width=1\linewidth]{murrell-xdvir_files/figure-latex/unnamed-chunk-33-1}
...
other drawing with the location of \LaTeX{} text.
...
is loaded in the authoring step, thatTi\textit{k}Z output is produced in the
...
We will see a more complex example ofTi\textit{k}Z output in a later section.
Figure \ref{fig:diag} is also aTi\textit{k}Z picture that has been rendered
in R.
...
for text labels tends to be more complex than
...
a \texttt{"\textbackslash{}LaTeX\{\}document"} object that is produced by
...
to typeset a \texttt{"\textbackslash{}LaTeX\{\}document"} that was produced for a different \TeX{} engine.
...
The \LaTeX{} package \texttt{annotate-equations} is built onTi\textit{k}Z graphics.
...
of the bounding box information fromTi\textit{k}Z graphics and, for images with
...
add to a \CRANpkg{ggplot2}, much like the standard \texttt{ggplot2::geom\_point()}
...
We pass to \texttt{latexGrob()} the \LaTeX{} code to draw (\texttt{schneiderTeX}),
...
This provides another demonstration of the range of possibilities that is
provided by
...
three colours, and then giving the title text, with the words
``Male'' and ``Female'' coloured differently.
...
a little way in from the left edge of the plot (to line up with
the left edge of the first column of panels), and we
load the \LaTeX{} package \texttt{xcolor} so that the colours work.
...
enumerated lists, multiple columns, andTi\textit{k}Z graphics.
...
converts R plots intoTi\textit{k}Z pictures so that R plots can include labels with
...
Murrell, Paul. 2015. {``The gridGraphics Package.''} \emph{The R Journal} 7: 151--62.
...
---------. 2025. {``Text in {R} Graphics.''} \emph{Australian \& New Zealand Journal of Statistics} {\emph{in press}}.
