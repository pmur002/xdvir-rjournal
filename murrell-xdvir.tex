...
\abstract{%
The \texttt{xdvir} package provides functions for rendering LaTeX code as labels, annotations, and data symbols in R plots. There are convenient high-level functions for rendering simple LaTeX fragments, including as labels on \texttt{ggplot2} plots, plus lower-level functions for more fine control over the authoring, typesetting, and rendering of LaTeX code. There is support for making use of LaTeX packages, including TikZ graphics. The rendered LaTeX output is fully integrated with R graphics output in the sense that LaTeX output can be positioned relative to R graphics output and vice versa.
}
...
\begin{figure}
\centering
\includegraphics{murrell-xdvir_files/figure-latex/typesetting-1.png}
\caption{\label{fig:typesetting}A plot with a text annotation that contains several typesetting challenges: in-line mathematical equations; changes in colour; and automated line-breaks with full justification and hyphenation.}
\end{figure}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-4-1.pdf}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-6-1.pdf}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-7-1.pdf}
...
\begin{figure}
\centering
\includegraphics{murrell-xdvir_files/figure-latex/elementlatex-1.png}
\caption{\label{fig:elementlatex}The \texttt{ggplot2} plot from Figure \ref{fig:typesetting}, without the text annotation, but with a title that was specified using a LaTeX fragment and the function \texttt{element\_latex()}.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{murrell-xdvir_files/figure-latex/geomlatex-1.png}
\caption{\label{fig:geomlatex}A \texttt{ggplot2} plot with text labels drawn using \texttt{geom\_latex()}.}
\end{figure}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-16-1.pdf}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-18-1.pdf}
...
\texttt{\textbackslash{}color} command would not be recognised; with the \texttt{xcolor} package,
the \texttt{\textbackslash{}color} command produces instructions to change colour
in the \texttt{"DVI"} output. This is demonstrated below with an
explicit call to the \texttt{typeset()} function. An example of the colour-change
...
same change in colour for the last two words.

\begin{verbatim}
render(colourDVI, packages="xcolor")
\end{verbatim}

\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-22-1.pdf}

There are several LaTeX packages with predefined support in the
\texttt{xdvir} package, including
\texttt{xcolor} for changes in colour and \texttt{fontspec} for changes in font.
...
For example, the following code draws the \texttt{tex} fragment vertically centred
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-23-1.pdf}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-24-1.pdf}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-25-1.pdf}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-27-1.png}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-28-1.png}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-30-1.pdf}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-31-1.pdf}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-32-1.pdf}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-33-1.pdf}
...
\begin{figure}
\centering
\includegraphics{murrell-xdvir_files/figure-latex/zref-1.png}
\caption{\label{fig:zref}The \texttt{ggplot2} plot from Figure \ref{fig:typesetting}, including the LaTeX annotation, with a line added relative to marked locations within the LaTeX annotation (and relative to the thick red line).}
\end{figure}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-35-1.pdf}
...
\includegraphics{murrell-xdvir_files/figure-latex/unnamed-chunk-37-1.pdf}

We will see a more complex example of TikZ output in a later section.

\hypertarget{customisation-and-debugging}{%
\section{Customisation and debugging}\label{customisation-and-debugging}}

Most of the examples in this article take a fragment of LaTeX code
and feed it to the \texttt{grid.latex()} function, which performs an
...
that allow us to perform these steps separately
(see Figure
\ref{fig:diagpdf}).

\begin{figure}
\centering
\includegraphics{diagram/diag.pdf}
\caption{\label{fig:diagpdf}The design of the \texttt{xdvir} package.}
\end{figure}
...
However, a LaTeX document within R is essentially just a character vector,
...
\begin{figure}
\centering
\includegraphics{murrell-xdvir_files/figure-latex/rahlf-1.png}
\caption{\label{fig:rahlf}A plot with a two-column text annoation. This plot is an adaptation of Figure 4.1 from Thomas Rahlf's book ``Data Visualisation with R'' (Rahlf 2017).}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{murrell-xdvir_files/figure-latex/rahlfplain-1.png}
\caption{\label{fig:rahlfplain}The main plot from Figure \ref{fig:rahlf} without the two columns of text annotation. This plot is drawn using functions from the \texttt{graphics} package.}
\end{figure}
...
\texttt{LaTeXpackage} to provide support for the LaTeX package \texttt{multicol}.
...
We also call \texttt{registerPackage()} so that we can refer to this LaTeX
package by its name.
...
demonstrates the sort of effects that are possible by
...
\begin{figure}
\centering
\includegraphics{murrell-xdvir_files/figure-latex/schneider-1.png}
...
One point worth noting is that the labelling on the x-axis, which
...
\begin{figure}
\centering
\includegraphics{murrell-xdvir_files/figure-latex/schneiderplain-1.png}
...
This is arranged in three blocks: the first block of code defines some colours;
...
This example, shown in Figure \ref{fig:anzjs},
provides another demonstration of the range of possibilities from
...
\begin{figure}
\centering
\includegraphics{murrell-xdvir_files/figure-latex/anzjs-1.png}
\caption{\label{fig:anzjs}A plot with numbered list items as annotations. This plot is an adaptation of Figure 10 from Murrell (2025).}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{murrell-xdvir_files/figure-latex/anzjsplain-1.png}
\caption{\label{fig:anzjsplain}The main plot from Figure \ref{fig:anzjs} without the numbered list items as annotations. This plot is produced using the \CRANpkg{ggplot2} package.}
\end{figure}
...
This section provides an example of using other software to generate
...
The final image is shown in \ref{fig:latticemd}.



\begin{figure}
\centering
\includegraphics{murrell-xdvir_files/figure-latex/latticemd-1.png}
\caption{\label{fig:latticemd}A plot with numbered list items as annotations. The list items in this plot are specified using Markdown rather than LaTeX.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{murrell-xdvir_files/figure-latex/latticemdplain-1.png}
\caption{\label{fig:latticemdplain}A plot with numbered list items as annotations. The list items in this plot are specified using Markdown rather than LaTeX.}
\end{figure}
...
instead of writing explicit LaTeX code, we use the arguably-simpler
Markdown syntax to
describe the list item. We will just focus on the left annotation;
...
to convert the Markdown to LaTeX code.
...
to which we provide a call to the \texttt{render()} function
from the \texttt{xdvir} package.
...
The \texttt{xdvir} package performs some caching in order to minimise the problem,
...
through a dependency on the \texttt{tinytex} package (Xie 2024).
...
The advantage of \texttt{xdvir} by comparison is that it is possible to
produce more advanced typesetting results with LaTeX.

The \CRANpkg{tikzDevice} package (\textbf{pkg:tikzDevice?}) is an interesting
alternative because, where \texttt{xdvir} integrates LaTeX text with R graphics,
\CRANpkg{tikzDevice} integrates R graphics with LaTeX.
...
where \texttt{xdvir} ends up with LaTeX output within an R plot,
\CRANpkg{tikzDevice} ends up with an R plot within LaTeX output.
...
demonstrated by (\textbf{schneider?}).
...
simpler make use of TeX engines,
...
Rahlf, Thomas. 2017. \emph{Data Visualisation with r: 100 Examples}. 1st ed. Springer Publishing Company, Incorporated.
