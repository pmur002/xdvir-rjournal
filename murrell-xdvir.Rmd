...
The simplest way to draw typeset text with the `xdvir` package 
...
the LaTeX logo.  Because LaTeX code tends to contain 
...
As the function name `grid.latex()` suggests, this is low-level 
drawing in the `grid` package graphics system.
The text is drawn relative to the current `grid` viewport.
We can add this low-level drawing to a plot in a number of ways.
For example, the following code uses the `grid_panel()` function
from the \CRANpkg{gggrid} package 
[@pkg-gggrid] to add a text label to the top-right corner of a 
\CRANpkg{ggplot2} plot [@pkg-ggplot2].
The `gg` object contains a description of the main \CRANpkg{ggplot2} 
plot from Figure 
\@ref(fig:typesetting).  The supplementary materials contains
full code for generating that object.
This code shows that we can create an object without drawing it,
using `latexGrob()`, and there are arguments for positioning 
the typeset text.  In this case we draw the text top-right justified
2mm in from the top-right corner of the plot region.
The object produced by `grid_panel()` is passed to \CRANpkg{ggplot2}
to be drawn as part of the plot.  

```{r}
library(gggrid)
```

```{r, dev="png", dev.args=list(type="cairo"), fig.width=7, fig.height=4.5, dpi=300}
gg +
...
```{r}
adventor <- fontspecPackage(font="TeX Gyre Adventor")
texStr <- r"(\definecolor{Rred}{RGB}{223,83,107}
\fontsize{12}{15}
\selectfont
This is \textcolor{Rred}{\LaTeX})"
```

```{r, dev="png", dev.args=list(type="cairo"), fig.width=7, fig.height=4.5, dpi=300}
gg +
    grid_panel(latexGrob(texStr,
                         x=unit(1, "npc") - unit(5, "mm"),
                         y=unit(1, "npc") - unit(5, "mm"),
                         hjust="right", vjust="top",
                         packages=list(adventor, "xcolor")))
```


# How the package works

# Drawing Figure \@ref(fig:typesetting)
...
tex <- author(readLines("schneider.tex"), packages=pkgs)
dvi <- typeset(tex, texFile="schneider-doc.tex")
...
```{r echo=FALSE}
tex <- author(readLines("longley.tex"), packages="fontspec")
dvi <- typeset(tex, texFile="longley-doc.tex")
par(mar=c(4, 4, 1, 1))
