...
In this case, the input is `r LaTeX`, a `r TeX` engine is used to
...
a simple `r LaTeX` mathematical expression, as shown below.
...
: The `typeset()` function in the `xdvir` package 
  ...
  produces a `"DVI"` object that contains instructions specifying the fonts to
...
`r LaTeX` packages.  As a simple example, changing the colour of text
...
`r LaTeX` command `\color` would not be recognized;  with the `xcolor` package, 
...
``r LaTeX`package()` function.  We will see other predefined packages
...
other drawing with the location of `r LaTeX` text.
...
However, we can now access the special locations in the `r LaTeX` output using the
...
a `"`r LaTeX`document"` object that is produced by
...
to typeset a `"`r LaTeX`document"` that was produced for a different `r TeX` engine.
...
We want to align the `r LaTeX` text with the main plot.  In particular, we want
...
We also call the `registerPackage()` function so that we can refer to this `r LaTeX`
package by its name.
...
```{r, schneider, echo=FALSE, out.width="100%", fig.cap='(ref:schneiderFigCap)'}
<<annotateequations>>
<<tikznobbox>>
<<roboto>>
<<anneq>>
...
The start of the `r LaTeX` code for the annotated equation is shown below
...
loaded from a local `TeX` directory.
...
add to a \CRANpkg{ggplot2}, much like the standard `ggplot2::geom_point()`
...
We pass to `latexGrob()` the `r LaTeX` code to draw (`schneiderTeX`), 
...
In other words, we will be specifying a width for the `r LaTeX` output
to fit into.
...
load the `r LaTeX` package `xcolor` so that the colours work.
...
\CRANpkg{tikzDevice} reverses the process and integrates R graphics with `r LaTeX`.
...
if we use \CRANpkg{tikzDevice} we end up with an R plot within `r LaTeX` output.
