...
The final result is shown in Figure \@ref(schneider).
...
generate table with xtable and include in lattice plot.
...
```{r diagsvg, eval=knitr::is_html_output(), results="asis", echo=FALSE, fig.cap="The design of the `xdvir` package."}
## Create the sort of structure that knitr::include_graphics() returns
structure("diagram/diag.svg", class= c("knit_image_paths", "knit_asis"), dpi=NULL)
```
...
Limitations:  speed (as always), only some devices, only some TeX engines

Future work:  look at what `marquee` is doing and try to keep up with that :)
