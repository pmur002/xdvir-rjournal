...
# Programmatic generation of LaTeX
...
LaTeX code.  For example, there are packages like \CRANpkg{xtable} [@pkg:xtable]
for generating LaTeX tables and the \CRANpkg{rmarkdown} package [@pkg:rmarkdown]
...
Figure \@ref(fig:lattice) of integrating `grid.latex()` output
...
(ref:latticeFigCap) A \CRANpkg{lattice} plot with LaTeX text used for the plot title and for annotations in each panel.
...
It then calls `grid.latex()` to draw
...
load the LaTeX package `xcolor` so that the colours work.
