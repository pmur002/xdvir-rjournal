...
abstract: >
  ...
  functions for rendering simple LaTeX fragments, including
  as labels on `ggplot2` plots, plus lower-level
...
(see Figure 
\@ref(fig:`r ifelse(knitr::is_html_output(), 'diagsvg', 'diagpdf')`)).

```{r diagpdf, eval=knitr::is_latex_output(), echo=FALSE, fig.cap="The design of the `xdvir` package."}
knitr::include_graphics("diagram/diag.pdf")
```

```{r diagsvg, eval=knitr::is_html_output(), echo=FALSE, fig.cap="The design of the `xdvir` package."}
...
```{r, latticemd, echo=FALSE, dev="png", dev.args=list(type="cairo"), fig.width=8, dpi=300, fig.cap='(ref:latticemdFigCap)'}
latticeFinal
```
...
```{r, latticemdplain, echo=FALSE, dev="png", dev.args=list(type="cairo"), fig.width=8, dpi=300, fig.cap='(ref:latticemdFigCap)'}
latticeFlights
```
...
to which we provide a call to the `render()` function
from the `xdvir` package.
...
The `xdvir` package performs some caching in order to minimise the problem,
...
through a dependency on the `tinytex` package [@pkg:tinytex].
...
The advantage of `xdvir` by comparison is that it is possible to
...
where `xdvir` ends up with LaTeX output within an R plot, 
...
The `xdvir` package depends on the \CRANpkg{tinytex} package
