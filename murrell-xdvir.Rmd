...
```{r, elementlatex, dev="png", dev.args=list(type="cairo"), fig.width=7, fig.height=4.5, dpi=300, fig.cap='(ref:elementlatexFigCap)'}
ggIntro +
...
```{r, zref, echo=FALSE, dev="png", dev.args=list(type="cairo"), fig.width=7, fig.height=4.5, dpi=300, fig.cap='(ref:zrefFigCap)'}
...
is loaded in the authoring step, TikZ output is produced in the
...
```{r, rahlfplain, echo=FALSE, dev="png", dev.args=list(type="cairo"), fig.width=12, fig.height=9, dpi=300, fig.cap='(ref:rahlfplainFigCap)'}
<<rahlfplot>>
```
...
We want to align the LaTeX text with the main plot.  In particular,
the top of the text is aligned with the value 14,000 on the y-scale
...
`LaTeXpackage` to provide support for the LaTeX package `multicol`.
...
We also call `registerPackage()` so that we can refer to this LaTeX
package by its name.
...
(ref:rahlfFigCap) A plot with a two-column text annoation. This plot is an adaptation of Figure 4.1 from Thomas Rahlf's book "Data Visualisation with R".
...
ggSchneider +
    grid_panel(annotation, 
               aes(x=x, y=y), 
               data=data.frame(x=160, y=dnorm(mu, mean=100, sd=15)))
```

The main plot in this example is a \CRANpkg{ggplot2} plot.
The details are not particularly relevant to this article, so the
...
```{r, schneiderplain, echo=FALSE, dev="png", dev.args=list(type="cairo"), dpi=300, fig.cap='(ref:schneiderplainFigCap)'}
ggSchneider
```
...
refers to one of the saved locations within the equation, in this case
...
Finally, we will use the LaTeX package `roboto` to access the 
...
\CRANpkg{ggplot2} plot.  In particular, we align the top of the LaTeX
output with the top of the density curve and we align the right side
of the LaTeX output with the right edge of the label "160" on the x-axis.

We saw in Section \@ref(userinterface) 
...
We give the call to `latexGrob()` the LaTeX code to draw,
in `schneiderTeX`, a set of `packages` to load, and arguments that position
...
```{r, schneider, echo=FALSE, dev="png", dev.args=list(type="cairo"), dpi=300, fig.cap='(ref:schneiderFigCap)'}
<<annotateequations>>
<<tikznobbox>>
<<roboto>>
<<anneq>>
...
```{r label1, echo=FALSE, eval=FALSE}
...
```{r label2, echo=FALSE, eval=FALSE}
label2 <- function(data, coords) {
    x1 <- coords$x[1]
    x2 <- coords$x[2]
    w <- convertWidth(unit(1 - x2, "npc") - unit(1, "mm"), "in")
    gap <- 15
...
```{r enumlistplot, echo=FALSE, eval=FALSE}
pushViewport(viewport(y=1, height=unit(3, "in"), just="top"))
print(ggEnumList, newpage=FALSE)
popViewport()
```
...
The plot has lines that extend below the main plot, so the following
code draws the \CRANpkg{ggplot2} plot within a `grid` viewport 
that occupies just the top 3 inches of the page.  This leaves room on the page
for the lines that extend below the plot.
...
to fit into.
The main plot produced by this code is shown in Figure \@ref(fig:anzjsplain).
...
```{r, anzjsplain, echo=FALSE, dev="png", dev.args=list(type="cairo"), fig.width=8, fig.height=4.5, dpi=300, fig.cap='(ref:anzjsplainFigCap)'}
<<anzjsplot>>
```

We will focus on the drawing of just one of the LaTeX annotations.
The LaTeX code for the left-hand label is shown below.
...
The `label1()` function calls `latexGrob()`, gives it the LaTeX code
to draw, in `closeTeX`, specifies the position for the LaTeX output,
...
The following code combines the left-hand label annotation with the
`ggANZJS` plot, adds a right-hand label as well, then draws the result
within a `grid` viewport in the top 3 inches of the page.
...
```{r, anzjs, echo=FALSE, dev="png", dev.args=list(type="cairo"), fig.width=8, fig.height=4.5, dpi=300, fig.cap='(ref:anzjsFigCap)'}
<<label1>>
<<label2>>
<<ggenumlist>>
<<enumlistplot>>
...
and feed it to the `grid.latex()` function, which performs an
authoring step, a typesetting step, and a rendering step.
We saw in  Section \@ref(underhood) that there are functions
`author()`, `typeset()`, and `grid.dvi()`
that allow us to perform these steps separately.
The authoring step involves generating a complete LaTeX document
from the LaTeX fragment that we pass as the first argument to `grid.latex()`.
For example, the following code and output shows the LaTeX fragment 
`colourTeX` from Section \@ref(packages) plus the LaTeX document that
is produced from that fragment by the `author()` function.
...
We can influence the additional content that is produced 
by the `author()` function through its arguments.
For example, the following call to the `author()` function adds
LaTeX code to load the LaTeX package `xcolor`
in the preamble of the LaTeX document.

```{r}
author(colourTeX, packages="xcolor")
```

However, the arguments to the `author()` function are only
designed to provide convenience for straightforward situations;
they do not provide
fine control over the contents 
of the LaTeX document.

For example ... use option [gray] for grayscale version ???
...
Thanks to Thomas Rahlf for permission to base the visualisation in 
Figure \@ref(fig:rahlf) on Figure 4.1 from his book
"Data Visualisation with R".

Thanks to Joel Schneider for publishing his work in an open format.
