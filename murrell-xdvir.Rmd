...
There are several LaTeX packages with predefined support, including 
`xcolor` for changes in colour and `fontspec` for changes in font,
and support can be added for other LaTeX packages with the
`LaTeXpackage()` function.  We will see an example of the latter 
in a later section.

# Justifying text

By default, the LaTeX text is centred upon a specified location.
...
grid.rect()
grid.segments(0, .5, 1, .5, gp=gpar(col="grey"))
grid.latex(tex, y=.5, vjust="bottom")
```
...
grid.rect()
grid.segments(0, .5, 1, .5, gp=gpar(col="grey"))
grid.latex(tex, y=.5, vjust="baseline")
```
...
which produces a more reliable baseline.  That baseline can be accessed, by
...
grid.lines(unit.c(curvePos$x, left$devx, right$devx),
           unit.c(curvePos$y, left$devy - unit(1, "mm"),
